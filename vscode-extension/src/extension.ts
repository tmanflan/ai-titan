...
import { AgentEnforcementService } from './agent-enforcement';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Initialize services
    pricingService = new PricingService(context.extensionPath);
    const enforcementService = new AgentEnforcementService(context);
...
async function generateCodeDetailed() {
    ...
    const prompt = await getDetailedInput(
        'What code would you like me to generate? (Detailed Mode)',
        'e.g., Create a function to sort an array\n\nYou can provide detailed specifications, requirements, examples, or any additional context (up to 25,000 characters)'
    );
...
async function askQuestion() {
    ...
    const question = await getDetailedInput(
        'Ask AI Titan a question',
...
async function showResponseInNewTab(content: string, title: string) {
...
async function getDetailedInput(prompt: string, placeholder: string): Promise<string | undefined> {
